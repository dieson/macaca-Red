...
(function (window, factory) {
    ...
    function Hilitor(id, tag, options) {
        var className = 'search_highlight';
        var colors = ["#FFA500"];
        ...
        this.setRegex = function (input) {
            input = input.replace(/[^\w0-9\\u\\:\\(\\)\\[\\]\\{\\}\\|\\$\\+\\.\\*\\?||^ ]+/g, "").replace(/\(+/g, '\\(').replace(/\)+/g, '\\)').replace(/\[+/g, '\\[').replace(/\]+/g, '\\]').replace(/\{+/g, '\\{').replace(/\}+/g, '\\}').replace(/\|+/g, '\\|').replace(/^}+/g, '\\^').replace(/\?+/g, '\\?').replace(/\++/g, '\\+').replace(/\*+/g, '\\*').replace(/\.+/g, '\\.').replace(/\$+/g, '\\$');
            var re = "(" + input + ")";
            if (!openLeft) re = "\\b" + re;
        ...
        this.hiliteWords = function (node) {
            ...
            if (skipTags.test(node.nodeName))
                return;
            if (node.nodeName === hiliteTag && node.className === className)
                return;
